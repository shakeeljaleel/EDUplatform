...
export default function StudentQuizRunnerPage({ params }: { params: Promise<{ quizId: string }> }) {
  ...
  const fetchQuiz = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Check if student has already submitted attempt
        if (data.quiz.attempts && data.quiz.attempts.length > 0) {
          setSubmissionResult({
            score: data.quiz.attempts[0].score,
            maxPossibleScore: data.quiz.questions?.reduce((a: number, q: any) => a + (q.maxMarks || q.points || 10), 0) || 100,
            status: data.quiz.attempts[0].status
          })
        }
      } else {
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Refresh quiz to get full graded answers if available
        fetchQuiz()
      } else {
  ...
  if (submissionResult) {
    ...
    const fallbackMax = quiz.questions?.reduce((a: number, q: any) => a + (q.maxMarks || q.points || 10), 0) || 100
    const maxScore = submissionResult.maxPossibleScore ?? fallbackMax
    const pct = submissionResult.percentage ?? (maxScore > 0 ? Math.round((score / maxScore) * 100) : 0)
    const stars = submissionResult.starsEarned ?? (pct >= 90 ? 5 : pct >= 75 ? 4 : pct >= 60 ? 3 : pct >= 40 ? 2 : 1)
    const fallbackHelix = Math.round((score / (maxScore || 1)) * 50)
    const helix = submissionResult.helixPointsEarned ?? fallbackHelix
    ...
    return (
      <div className="content-wrapper" style={{ maxWidth: '800px' }}>
        <div className="card" style={{ background: '#ffffff', border: '3px solid #1a1a2e', borderRadius: '20px', boxShadow: '8px 8px 0px #1a1a2e', padding: '2.5rem', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '2rem', fontWeight: 900, color: '#1a1a2e', marginBottom: '0.5rem' }}>Quiz Completed!</h2>
          ...
          <div style={{ background: '#f8fafc', border: '3px solid #1a1a2e', borderRadius: '16px', padding: '1.5rem', marginBottom: '2rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1rem' }}>
            ...
            <div>
              <div style={{ fontSize: '0.75rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Rating</div>
          ...
          {quiz.showAnswersAfterSubmission && (
            <div style={{ textAlign: 'left', marginTop: '2rem' }}>
              <h3 style={{ fontSize: '1.3rem', fontWeight: 900, color: '#1a1a2e', marginBottom: '1rem' }}>Question Breakdown & Feedback</h3>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginTop: '0.5rem' }}>
                          {opts.map((opt: any, oIdx: number) => {
                            ...
                            return (
                              ...
                              >
                                {String.fromCharCode(65 + oIdx)}. {opt.text} {isCorrect ? '✓ Correct' : isSelected ? '❌ Your answer' : ''}
                              </div>
                            )
                          })}
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.5rem' }}>
                          ...
                          {studentAns.aiFeedback && (
                            <div style={{ fontSize: '0.85rem', fontWeight: 700, background: '#f0fdf4', color: '#15803d', padding: '0.75rem', borderRadius: '8px', border: '1.5px solid #00c853' }}>
                              🤖 <strong>AI Grading Feedback:</strong> {studentAns.aiFeedback}
                            </div>
                          )}
                        </div>
                      )}
            ...
            {questions.map((q: any, i: number) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: hasAnswered ? '#f0fdf4' : '#fff5f5',
                    border: '2px solid #1a1a2e',
                    borderRadius: '12px',
                    padding: '1rem',
                    cursor: 'pointer',
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600, marginTop: '0.2rem' }}>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
            <div style={{ maxWidth: '450px', width: '100%', background: '#ffffff', border: '3px solid #1a1a2e', borderRadius: '20px', boxShadow: '8px 8px 0px #1a1a2e', padding: '2rem', textAlign: 'center' }}>
              ...
              <p style={{ color: '#64748b', fontWeight: 600, marginBottom: '1.5rem' }}>
                You cannot change your answers after submitting. Are you sure you want to finish now?
              </p>
              <div style={{ display: 'flex', gap: '1rem' }}>
                <button
